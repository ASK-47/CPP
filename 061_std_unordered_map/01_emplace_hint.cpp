...
std::size_t map_emplace() {//just emplace
    std::unordered_map<int, char> map;
    for (int i = 0; i < n_operations; ++i) map.emplace(i, 'a');
    return map.size();
}

std::size_t map_emplace_hint() {//++i, emplace into end =>right order
    std::unordered_map<int, char> map;
    auto it = map.begin();
...
std::size_t map_emplace_hint_wrong() {//--i, emplace into end =>wrong order
    std::unordered_map<int, char> map;
    auto it = map.begin();
...
std::size_t map_emplace_hint_corrected() {//--i, emplace into bigin =>right order
    std::unordered_map<int, char> map;
    auto it = map.begin();
    for (int i = n_operations; i > 0; --i) {
...
std::size_t map_emplace_hint_closest() {//no temp it
    std::unordered_map<int, char> map;
    auto it = map.begin();
    for (int i = 0; i < n_operations; ++i) it = map.emplace_hint(it, i, 'e');
    return map.size();
}

double time_it(std::function<std::size_t()> map_test, std::string what = "",
               double ratio = 0.0) {
    ...
    if (what.size() && map_size)
        std::cout << std::setw(8) << time.count() << " for " << what
                  << " (ratio: " << (ratio == 0.0 ? 1.0 : ratio / time.count())//insetions per tact (millsecond) == insertion speed
                  << ")\n";
    return time.count();
}
